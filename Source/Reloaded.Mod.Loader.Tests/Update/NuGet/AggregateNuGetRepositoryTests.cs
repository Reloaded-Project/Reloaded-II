...
namespace Reloaded.Mod.Loader.Tests.Update.NuGet
{
    public class AggregateNuGetRepositoryTests
    {
        ...
        private static readonly PackageIdentity Skin = new PackageIdentity("sonicheroes.skins.midnighthill", NuGetVersion.Parse("1.0.0"));

        // Dependency in same source (Hooks)
        private static readonly PackageIdentity Redirector = new PackageIdentity("reloaded.universal.redirector", NuGetVersion.Parse("2.0.0"));

        // Missing Dependency + In Another Source (Hooks)
        private static readonly PackageIdentity Missing = new PackageIdentity("riders.tweakbox", NuGetVersion.Parse("4.0.0"));

        // No Dependencies
        private static readonly PackageIdentity Hooks = new PackageIdentity("reloaded.sharedlib.hooks", NuGetVersion.Parse("1.3.0"));
